#fitting models with gbif climate.
...
#set output path.----
model.output.path <- lm_pgls_means_myc.pgf_gbif_models.path
summary.output.path <- lm_pgls_means_myc.pgf_gbif_summary.path
...
#gbif run. rename gbif temp and precip to mat.c and map.c so downstream functions all work.
d$mat.c <- NULL
d$map.c <- NULL
setnames(d,c('gbif_temp','gbif_precip'),c('mat.c','map.c'))
