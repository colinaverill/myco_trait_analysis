...
#set output path.----
output.path <- pgls.glmm_myc.biome3_interaction.path
...
#traits <- c('Ngreen','Nsenes','Nroots','Pgreen','Psenes','Proots','log.LL','root_lifespan','Nresor','Presor')
#traits <- c('Ngreen','root_lifespan') #for testing.
...
for(i in 1:length(traits)) {
  ...
  #leaf lifespan already log10 transformed.
  if(traits[i] == 'log.LL'){
    form <- paste0(traits[i],' ~ ', predictors)
  }
  #root lifespan is special (not enough data for all interactions, zero nfixers.)
  if(traits[i] == 'root_lifespan'){
    form <- paste0('log10(',traits[i],') ~ MYCO_ASSO + biome_bore + pgf + mat.c + map.c')
  }
