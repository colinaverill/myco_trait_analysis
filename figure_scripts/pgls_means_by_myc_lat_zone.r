...
#To deal:
#5. Get posthoc significance when you hear from Will.
#6. Get correct error bars when you hear from Will.
rm(list=ls())
source('paths.r')
...
#load data.----
d <- readRDS(pgls.glmm_myc.biome3_interaction.path)
...
for(i in 1:8){
  ...
  lat.lab <- c('boreal-arctic','temperate','tropical')
  ...
  if(i == 2){
    ...
    p.lab[c(1,3)] <- ''
    lat.lab[3] <- ''
    N.val <- d[[i]]$sample_size
    N.lab <- c('',N.val[2],N.val[1],N.val[3])
    bore.N <- c('NA',d[[i]]$sample_size[c(2)])
    temp.N <- d[[i]]$sample_size[c(1,3)]
    trop.N <- c('','')
    N.lab <- c(paste(bore.N, collapse = ', '),
               paste(temp.N, collapse = ', '))
    N.lab <- paste0('N = (',N.lab,')')
  ...
  text(lat.lab, x = lat.lab.pos, y = limy[2] * 0.18)
  text(  p.lab, x = lat.lab.pos, y = limy[2] * 0.12)
  text(  N.lab, x = lat.lab.pos, y = limy[2] * 0.05)
  #trait label.
