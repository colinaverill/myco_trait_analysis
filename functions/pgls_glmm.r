...
pgls_glmm <- function(pgls.formula, d, phy){
  ...
  if(!('root_lifespan' %in% to_keep)){
    ...
    #check if any biome interactions are significant.
    biome_interaction <- sum.model$solutions[grep('MYCO_ASSOECM:',rownames(sum.model$solutions)),]
    biome_interaction_drop <- biome_interaction[,'pMCMC'][biome_interaction[,'pMCMC'] > 0.05]
  }
  #hack so code works with root_lifespan.
  if('root_lifespan' %in% to_keep){biome_interaction_drop <- c(1)}
  
  #drop biome interactions that are not "significant".----
  if(length(biome_interaction_drop) != 0){
    cat('Some myco-biome interaction not significant. Refitting model without interaction...\n')
    for(i in 1:length(biome_interaction_drop)){
      if('root_lifespan' %in% to_keep){break} #again, hack so root_lifespan works.
      check <- gsub('ECM','',names(biome_interaction_drop)[i])
      check <- paste0(check,' +')
      pgls.formula <- as.formula(gsub(check,'', format(pgls.formula), fixed =T))
    }
    model <- MCMCglmm(pgls.formula, random=~Species, ginverse=list(Species=rnd), data=dat, prior = priors)
    sum.model <- summary(model)
    sum.model <- data.frame(sum.model$solutions)
    #Any biomes p < 0.05 we should drop?
    biome_drop <- rownames(sum.model[sum.model$pMCMC > 0.05,])
    biome_drop <- biome_drop[grep('biome',biome_drop)]
    #If the interaction is still present, don't drop that biome.
    if(length(biome_drop) > 0){
      for(i in 1:length(biome_drop)){
        check <- biome_drop[grep(biome_drop[i],names(biome_interaction_drop))]
        if(length(check) == 0){
          biome_drop <- biome_drop[!(biome_drop %in% biome_drop[i])]
        }
      }
    }
  }
  
  #drop biome effects that are not "significant".----
  if(length(biome_interaction_drop) != 0){
    if(length(biome_drop) != 0){
      cat('Some biome main effects are not significant. Refitting model without these biomes...\n')
      for(i in 1:length(biome_drop)){
        check <- paste0(biome_drop[i],' +')
        pgls.formula <- as.formula(gsub(check,'',format(pgls.formula), fixed =T))
      }
      model <- MCMCglmm(pgls.formula, random=~Species, ginverse=list(Species=rnd), data=dat, prior = priors)
      sum.model <- summary(model)
